...
# DI
output_di_canada <- merge(output_di_canada, di_customers, all.x = T, by = 'sf_id_di')
output_di_canada <- output_di_canada[, is_current_customer:=ifelse(is.na(account_name), 0, 1)]
output_di_canada <- output_di_canada[, account_name:=NULL]
...
output_di_canada <- output_di_canada[, max_score:=as.numeric(as.character(max_score))]
output_di_canada <- output_di_canada[, dealership_name_di:=as.character(dealership_name_di)]
output_di_canada <- output_di_canada[, dealership_name_atlas:=as.character(dealership_name_atlas)]
output_di_canada <- output_di_canada[, website_di:=as.character(website_di)]
output_di_canada <- output_di_canada[, dealership_website_atlas:=as.character(dealership_website_atlas)]
output_di_canada$random_val <- runif(nrow(output_di_canada), min=0.9, max=0.99)
output_di_canada <- output_di_canada[, max_score:=ifelse(max_score < 0.9 & dealership_name_di == dealership_name_atlas, random_val, max_score)]
output_di_canada <- output_di_canada[, max_score:=ifelse(max_score < 0.9 & website_di == dealership_website_atlas & (dealership_website_atlas!=' '), random_val, max_score)]
#hist(output_di_canada$max_score, breaks = 100)
output_di_canada$random_val <- NULL

write.csv(output_di_canada, file = "output_di_canada.csv", row.names = F)
...
# DI
di_customers <- read.csv("di_customers.csv", stringsAsFactors=F)
di_usa_data <- do.call("rbind", list(output_di_usa_post_al, output_di_usa_post_fl,
output_di_usa_post_ne, output_di_usa_post_tn
))
...
output_di_usa <- merge(output_di_usa, di_customers, all.x = T, by = 'sf_id_di')
output_di_usa <- output_di_usa[, is_current_customer:=ifelse(is.na(account_name), 0, 1)]
output_di_usa <- output_di_usa[, account_name:=NULL]
...
#output_di_usa[(max_score <0.8) & (as.character(website_di) == as.character(dealership_website_atlas)) & (as.character(dealership_website_atlas)!=' '), .(dealership_name_atlas, dealership_name_di, department_address_atlas, dealership_address_di)]
output_di_usa <- output_di_usa[, dealership_name_di:=as.character(dealership_name_di)]
output_di_usa <- output_di_usa[, dealership_name_atlas:=as.character(dealership_name_atlas)]
output_di_usa <- output_di_usa[, website_di:=as.character(website_di)]
output_di_usa <- output_di_usa[, dealership_website_atlas:=as.character(dealership_website_atlas)]
output_di_usa$random_val <- runif(nrow(output_di_usa), min=0.9, max=0.99)
output_di_usa <- output_di_usa[, max_score:=ifelse(max_score < 0.9 & dealership_name_di == dealership_name_atlas, random_val, max_score)]
output_di_usa <- output_di_usa[, max_score:=ifelse(max_score < 0.9 & website_di == dealership_website_atlas & (dealership_website_atlas!=' '), random_val, max_score)]
#hist(output_di_usa$max_score, breaks = 100)
output_di_usa$random_val <- NULL

write.csv(output_di_usa, file = "output_di_usa.csv", row.names = F)
